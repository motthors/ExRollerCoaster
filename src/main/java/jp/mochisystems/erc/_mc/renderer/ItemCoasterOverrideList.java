...
public class ItemCoasterOverrideList extends ItemOverrideList {

    public ItemCoasterOverrideList(List<ItemOverride> overridesIn)
    {
        super(overridesIn);
    }
        ...
        if (!stack.isEmpty() && stack.getItem().hasCustomProperties())
        {
            ...
            if (!this.getOverrides().isEmpty()) {
                for (ItemOverride itemoverride : this.getOverrides()){
                    ...
                    String name = ModelPackLoader.Instance.GetCoasterIconName(id);
